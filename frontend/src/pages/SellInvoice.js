...
export default function SellInvoice() {
    ...
    useEffect(() => {
        const fetchBuyers = async () => {
            try {
                ...
                setBuyers(res.data); // 👈 اضافه شد
            } catch (err) {
                console.error("خطا در گرفتن خریداران:", err);
            }
        };
        fetchBuyers();
    }, []);
    ...
    const createInvoiceItems = async (invoiceId) => {
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const itemPayload = {
                sell_invoice: invoiceId, // 👈 تغییر مهم
            ...
            try {
                const res = await axiosInstance.post("/sell-invoice-items/", itemPayload);
    ...
    const selectedBuyer = buyers.find(b => b.id === parseInt(buyerId)); // 👈 اصلاح شد
