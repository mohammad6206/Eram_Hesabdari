...
export default function Seller() {
  ...
  const fetchNextNumber = async () => {
    try {
      const res = await axiosInstance.get(`${API_URL}/api/next-number/Seller/`);
      setSelectedSeller((prev) => ({
        ...prev,
        number: res.data.next_number,
      }));
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (e) => {
    ...
    // ⬅️ تبدیل تاریخ شمسی → میلادی برای ذخیره
    const payload = {
      ...selectedSeller,
      created_at: selectedSeller.created_at
        ? selectedSeller.created_at.toDate().toISOString()
        : null,
    };

    try {
      await axiosInstance.post(`${API_URL}/api/sellers/`, payload, {
  ...
  // ⬅️ اگر از API میلادی گرفتی → تبدیل به شمسی برای DatePicker
  const loadSeller = async (id) => {
    try {
      const res = await axiosInstance.get(`${API_URL}/api/sellers/${id}/`);
      const data = res.data;
      setSelectedSeller({
        ...data,
      ...
      <div className="mb-3 d-flex justify-content-between align-items-center">
        <div className="col-md-5 d-flex align-items-center gap-2">
          ...
          <DatePicker
            ...
            onChange={(date) =>
              setSelectedSeller((prev) => ({ ...prev, created_at: date }))
            }
          ...
          />
          {errors.created_at && (
            <div className="text-danger small">{errors.created_at}</div>
          )}
        </div>
      ...
      <form onSubmit={handleSave} className="row">
        {[
          ...
          <div className="col-md-6 mb-3" key={item.field}>
            ...
            />
            {errors[item.field] && (
              <div className="text-danger">{errors[item.field]}</div>
            )}
          </div>
        ))}
